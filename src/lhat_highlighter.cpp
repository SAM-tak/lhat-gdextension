...
#include "lhat.h"
#include "lhat_host.h"
#include "lhat_icon.h"  // generated by CMake from godot/lhat-script.svg
...
{
    // Connected to below rather than called: a theme is rebuilt when the
    // editor's own settings change, and what was added to the old one is not
    // on the new one.
    ClassDB::bind_method(D_METHOD("hang_the_icon"),
                         &LhatEditorPlugin::hang_the_icon);
    // Both connected to and deferred to; see asked_for_receiver.
    ClassDB::bind_method(D_METHOD("asked_for_receiver", "object", "function",
                                  "arguments"),
                         &LhatEditorPlugin::asked_for_receiver);
    ClassDB::bind_method(D_METHOD("write_receiver", "script", "function",
                                  "arguments"),
                         &LhatEditorPlugin::write_receiver);
}

// 02 の 14.13 with the editor's own way of doing this: the connect dialog
// asks for a receiver by signalling the editor, and what answers first puts
// it at the end of the file (lhat_receiver.h says why that is no good). This
// answers second and writes the member where it belongs.
//
// Deferred rather than done here, so that it does not matter which of the
// two was connected first: what runs after the whole emit has unwound is
// after the editor's insertion and after the save it does beside it.
void LhatEditorPlugin::asked_for_receiver(Object *object, const String &function,
                                          const PackedStringArray &arguments)
{
    if (object == nullptr) {
        return;
    }
    // Every language's request comes through this one signal.
    Ref<Script> attached = object->get_script();
    LhatScript *ours = Object::cast_to<LhatScript>(attached.ptr());
    if (ours == nullptr) {
        return;
    }
    call_deferred("write_receiver", attached, function, arguments);
}

void LhatEditorPlugin::write_receiver(const Ref<Script> &script,
                                      const String &function,
                                      const PackedStringArray &arguments)
{
    lhat_receiver_write(Object::cast_to<LhatScript>(script.ptr()), function,
                        arguments);
}

// What the editor draws a .lh with: the Create Script dialog's language list
// and the file browser both ask the editor theme for an icon named after the
// resource type (LhatScript, which lhat_resource_format.cpp answers). There
// is none, so both showed the missing-icon square.
//
// Put on the theme rather than declared, since a GDExtension has no way to
// say "here is my language's icon" -- Godot's own languages are compiled in
// with their icons. Rasterised at the editor's scale, the way the engine
// rasterises its own: an icon frozen at one size is the wrong size on any
// other display.
void LhatEditorPlugin::hang_the_icon()
{
    EditorInterface *editor = EditorInterface::get_singleton();
    if (editor == nullptr) {
        return;
    ...
    // The drawing is 64 wide; an editor icon is 16. The scale carries the
    // display's, so a 200% editor gets 32 real pixels rather than a blur.
    const float wanted = 16.0f * editor->get_editor_scale();
    Ref<Image> drawn;
    drawn.instantiate();
    if (drawn->load_svg_from_string(String::utf8(LHAT_SCRIPT_ICON_SVG),
                                    wanted / 64.0f) != OK) {
        return;
    }
    theme->set_icon("LhatScript", "EditorIcons",
                    ImageTexture::create_from_image(drawn));
}

void LhatEditorPlugin::_enter_tree()
{
    hang_the_icon();
    // 4.x rebuilds the editor theme when a setting touches it -- the colour
    // preset, the display scale -- and the icon goes with the old one.
    EditorInterface *editor = EditorInterface::get_singleton();
    Ref<EditorSettings> settings =
        editor != nullptr ? editor->get_editor_settings() : Ref<EditorSettings>();
    if (settings.is_valid() &&
        !settings->is_connected("settings_changed",
                                Callable(this, "hang_the_icon"))) {
        settings->connect("settings_changed", Callable(this, "hang_the_icon"));
    }

    // 02 の 18.7's connect dialog asks the editor for a receiver, and the
    // editor is where the request is signalled from -- gui_base is its own
    // child, so its parent is the one holding the signal.
    Node *editor_node =
        editor != nullptr && editor->get_base_control() != nullptr
            ? editor->get_base_control()->get_parent()
            : nullptr;
    if (editor_node != nullptr &&
        editor_node->has_signal("script_add_function_request") &&
        !editor_node->is_connected("script_add_function_request",
