...
# space after one -- so nothing below puts a space or a bracket there, and a
# regeneration leaves what the formatter wrote alone.
...
def write_lh(api, classes, wanted, rows):
    out = []
    say = out.append
    say("# GENERATED by scripts/gen-godot-api.py. Do not edit.")
    say("#")
    ...
    say("#")
    say("# delegate^, which 02 の 14.7改2 spells, is what shows a class's")
    say("# members through the wrapper. It makes no forwarding procedures")
    say("# at all -- the name is looked for through the handle -- so this")
    say("# file costs a few lines per class however many methods it has.")
    say("#")
