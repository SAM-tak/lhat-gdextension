...
| `import^ std.` | 登録済みモジュール | **ホスト** |
| `require^ "li` | 単位（ファイル） | **ホスト** |

前2つはコアが検査器に訊いて答える——`lhat_type_find_member` の答えそのもの、
スコープは各スコープが閉じたときの記録そのもの。**探索をやり直さない**のが要点で、
14.10 の探索や 8 章のスコープをここで読み直せば、規則が難しいところで検査器と
食い違う。後ろ2つの一覧はホストのもので、コアは**問いの種別と書きかけの開始位置**
だけを答える。
...
［補足］ 実測（`good.lh`、1回あたり）: コメントの中 7µs、語 338µs、
`self^.`（212件）801µs、`godot.Node` のメンバ（216件）3.8ms。数える呼びと
