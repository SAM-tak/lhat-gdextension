...
Ctrl+Space の候補は `lhat/completion.h` から来る。07 の 4 章が言うとおり
**一つの要求に四つの問い**が着ていて、カーソルの左に何が書いてあるかで分かれる:

| 左にあるもの | 問い | 誰が答えるか |
| --- | --- | --- |
| `foo.` | レシーバのメンバ | コア（検査器の記録） |
| `le` | スコープの名前と言語の語 | コア（同上） |
| `import^ std.` | 登録済みモジュール | コア（登録そのもの） |
| `require^ "li` | 単位（ファイル） | **ホスト** |

最初の2つはコアが検査器に訊いて答える——`lhat_type_find_member` の答えそのもの、
スコープは各スコープが閉じたときの記録そのもの。**探索をやり直さない**のが要点で、
14.10 の探索や 8 章のスコープをここで読み直せば、規則が難しいところで検査器と
食い違う。モジュールもコアが持つ——プログラムは自分に何が登録されたかを知って
いるので、ホストが写しを持つ理由が無い。残るのは単位の一覧だけで、そこはコアが
**問いの種別と書きかけの開始位置**だけを答える。

カーソル位置は仮想関数の引数に無い。エンジンは本文の中に **U+FFFF** を1文字
差し込んで渡す（`code_edit.cpp`）ので、それを抜いた本文と、そのバイト位置が
コアに渡すものになる。

メンバだけは検査器が要る——点が「アクセス」だったのか「小数点」だったのかを
知っているのは検査器だけなので。他の3つは本文だけで答わるので、検査を払う前に
`lhat_completion_ask_text` で分岐する。

コアが返すのは「接頭辞の続き」なので、`import^g` に対しては `odot` が返る。
エディタはカーソルから後ろに読んだ語（`g`）で絞り込むので、**綴り全体に戻して**
から渡す。単位の候補も同じ理由で1区切りずつ出す。

［補足］ 実測（1回あたり、本文が変わらない場合）: コメントの中 8µs、語 422µs、
`self^.`（212件）657µs、`godot.Node` のメンバ（216件）657µs。数える呼びと
埋める呼びを分けると倍かかるので、256件ぶんの場所を先に取って1回で済ませている。
